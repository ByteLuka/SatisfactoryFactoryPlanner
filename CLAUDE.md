...
**Byproduct detection** (`computeEdges` in `solver.worker.ts`): an item is a byproduct when it has recipe producers, is not a production target, and either has no downstream consumers *or* produces more than downstream consumers need (surplus). Both cases route to a `byproduct_<itemClassName>` sink node; surplus is the difference between total producer output and total consumer input. `ByproductNode` appears as a terminal node on the right side of the graph.
