...
Three power fields also pass through from raw to domain `Recipe`: `isVariablePower`, `minPower`, `maxPower`. Variable-power recipes (Particle Accelerator, Converter, Quantum Encoder) ignore the building's `powerConsumption` baseline and instead draw between `minPower` and `maxPower` MW per machine. The domain `Building` type exposes `powerConsumption`, `powerConsumptionExponent`, and `powerProduction` (all read from `metadata.*` in `buildings.json`; exponent is `~1.321928` for all current buildings). The underclocking formula is `power = basePower × (clockSpeed/100)^exponent`; `Phase2Page.tsx`'s `powerStats` memo applies this to compute four Plan Summary statistics (min/max × all-at-100% / with-underclock) plus `totalPowerProduced`.

**Plan Summary — resource usage percentages:** The Plan Summary sidebar section renders each resource from `activePlan.resourceUsage` with a color-coded percentage badge showing consumption relative to `MAP_RESOURCE_POOL_LIMITS`. Thresholds: green < 40%, yellow 40–80%, red ≥ 80%. A `title` tooltip shows the full detail (`used / global limit (pct%)`). The color thresholds are intentional game-balance choices — do not change them without reason.

**Synthesized recipes — nuclear generators:** `FGBuildableGeneratorNuclear` defines fuel consumption via `mFuel` arrays, not as `FGRecipe` entries in Docs.json. The preprocessor synthesizes `GeneratorFuel_<fuelClassName>` recipes for each fuel entry and links them to the schematic that unlocks the fuel rod's manufacturing recipe. `Build_GeneratorNuclear_C` has `powerConsumption: 0` and `powerProduction: 2500` — it is a power source, not a consumer. The `powerStats` memo in `Phase2Page` branches on `building.powerProduction > 0` and accumulates `totalPowerProduced` with a `continue` that skips the normal consumption path — do not merge these two branches.
...
- `OptimizationStrategy`:
  - `MAX_OUTPUT` — maximizes net production of unrated targets; auto-selected when any target has no rate. After the LP solves, a second BALANCED pass re-runs with the achieved output rates fixed — this eliminates LP degeneracy where recipes with zero objective coefficient (no contribution to the target) can be set to arbitrary non-zero values by the simplex, producing spurious machine activity.
...
- **Raw resource seeding invariant:** `rawResources` is seeded from `Object.keys(resourcePool.limits)` *before* scanning recipe ingredients. This ensures items like Water — which appear as products of the Unpackage Water recipe — are classified as raw resources rather than balance-constrained producible items. `solverProducibleItems = producibleItems.filter(cn => !rawResources.has(cn))` is what's actually passed to the LP and to `buildInfeasibilityMessage`. All calls to `buildAndSolve` and `greedyEliminate` must use `solverProducibleItems`, not the original `producibleItems`.
- **Raw resource output exclusion:** When building `planNodes`, products that are raw resources are skipped (`if (rawResources.has(product.itemClassName)) continue`). Without this, a packaging recipe like Unpackage Water adds Water to `itemFlows.producers`, making `producers.length > 0`, which causes `isRawResource` to evaluate `false` in `computeEdges` — suppressing the `resource_<item>` graph node and its edges entirely.
...
**Byproduct detection** (`computeEdges` in `solver.worker.ts`): an item is a byproduct when it has recipe producers (in `planNodes.outputRates` — raw resource products are excluded, see above), is not a production target, and either has no downstream consumers *or* produces more than downstream consumers need (surplus). Both cases route to a `byproduct_<itemClassName>` sink node; surplus is the difference between total producer output and total consumer input. `ByproductNode` appears as a terminal node on the right side of the graph.
...
- **Edge flow assignment:** sources and sinks for each item are sorted by rate descending and matched with a greedy two-pointer — the largest source drains into the largest sink before either advances. This produces at most `sources + sinks − 1` edges per item (vs. `sources × sinks` for proportional splitting) and keeps whole-producer connections where possible (e.g. a refinery that alone covers a downstream consumer connects to it cleanly rather than being split across multiple sinks).
...
- `displayEdges` also sets `data.isIncoming = true` on highlighted edges whose **target** is the selected node. `ELKRouteEdge` uses this to synchronize the flowing-dash animation on converging edges: for outgoing edges the animation is anchored at the source (position 0 of the path), which naturally synchronizes diverging edges; for incoming edges the flow `<path>` is built from the **reversed** point array and the CSS animation plays with `animation-direction: reverse` so dashes still visually flow source→target but are now anchored at the target end. Do not remove this reversal — without it, converging edges that share a path segment near the target display interleaved dashes that fill each other's gaps.
